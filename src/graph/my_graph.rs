...
impl MyGraph {
    ...
    pub fn reduce(self) -> MySGraph {
        let mut representatives = HashMap::new();
        let mut twins_set = Vec::new();

        let mut rest = self.nodes;
        let mut idcs = (0u32..rest.len().try_into().unwrap()).collect::<Vec<_>>();
        while let (Some(node), Some(idx)) = (rest.pop(), idcs.pop()) {
            ...
            'outer: for (tp, (other, &ti)) in enumerate!(rest.iter().zip(idcs.iter())) {
            ...
            if !twins_idx.is_empty() {
                let neighbors = {
                    ...
                    idcs.swap_remove(tp as usize);
                    rest.swap_remove(tp as usize)
                };
        ...
        MySGraph { nodes: representatives }
    }
}

/// The neighbors are sorted.
impl<'l> FromIterator<&'l Operator> for MyGraph {
    fn from_iter<T: IntoIterator<Item = &'l Operator>>(iter: T) -> Self {
        let iter = iter.into_iter();
        let size = iter.size_hint().0;
        let mut ops: Vec<&Operator> = Vec::with_capacity(size);
        let mut nodes: Vec<VNeighbourhood> = Vec::with_capacity(size);

        for (i, op) in (0..).zip(iter) {
            let mut node = VNeighbourhood::new();
            for (j, other_op) in (0..).zip(ops.iter()) {
                if !op.commute(other_op) {
                    node.push(j);
                    nodes[j as usize].push(i)
                }
            }
            nodes.push(node);
            ops.push(op);
...
/// The neighbors are sorted.
impl<O: Borrow<Operator>> From<&[O]> for MyGraph {
    fn from(ops: &[O]) -> Self {
        let iter = ops.iter();
        let mut nodes: Vec<VNeighbourhood> = Vec::with_capacity(ops.len());

        for (i, op) in enumerate!(iter) {
            let mut node = VNeighbourhood::new();
            for (j, other_op) in enumerate!(ops[..i as usize].iter()) {
...
impl MySGraph {
    ...
    pub fn raw_node_claw_count(&self, neighbors: &HNeighbourhood) -> Vec<u32> {
        if neighbors.len() < 3 {
            return Vec::new();
        }
...
mod tests {
    ...
    fn from_sanity() {
        ...
        assert_eq!(
            MyGraph::from_iter(sample.iter().map(Deref::deref)),
            MyGraph::from(sample.as_slice())
        );
    }
    ...
    fn reduction() {
        ...
        for (origin, expected) in hamiltonians.into_iter() {
            let graph = MyGraph { nodes: origin };
