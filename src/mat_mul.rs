...
impl Matrix {
    pub fn from_vec_with_shape(vec: Vec<int>, shape: (usize, usize)) -> Self {
    ...
    pub fn get(&self, i: usize, j: usize) -> &int {
        self.data.get(self.idx(i, j)).unwrap()
    }

    pub fn get_mut(&mut self, i: usize, j: usize) -> &mut int {
        let idx = self.idx(i, j);
        self.data.get_mut(idx).unwrap()
    }

    /// # Safety
    pub unsafe fn get_unchecked(&self, i: usize, j: usize) -> &int {
        unsafe { self.data.get_unchecked(self.idx(i, j)) }
    }

    /// # Safety
    pub unsafe fn get_mut_unchecked(&mut self, i: usize, j: usize) -> &mut int {
        let idx = self.idx(i, j);
